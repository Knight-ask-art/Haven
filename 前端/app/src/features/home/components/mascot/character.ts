/* eslint-disable */
// @ts-nocheck
import { GROK_GEO } from './geometry-data';
import { GROK_MATH as M } from './math';
import { GROK_TABLES as T } from './tables';
import { GROK_POSE } from './pose';
import { GROK_TRICKS } from './tricks';
import { GROK_EYES } from './eyes';
import { GROK_FX } from './fx';


  
  const { applyPose, nextGaze } = GROK_POSE;
  const TR = GROK_TRICKS;
  const EY = GROK_EYES;
  const FX = GROK_FX;
  const {
